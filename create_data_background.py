...
network_spec = generate_network.from_file_to_network_spec(file_name)
data_fr = generate_network.create_clean_data(network_spec)
data_fr = generate_network.add_noise(data_fr, network_spec["noise_mean"], network_spec["noise_std"])

data_fr.to_csv(where_to_dir + f"data_for_{file_name.split('.')[0]}.csv")

subsets_to_calculate = findsubsets(data_fr.columns, 2)[start:end]
for pair in :
