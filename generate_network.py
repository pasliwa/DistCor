...
def create_network_from_dict(network_spec):
    ...
    number_of_generating_functions = int(network_spec["number_of_generating_functions"])
...
def create_clean_data(network_spec):
    """
    Network_spec partial description:
    variable_names - list
    generating_functions - list of triples
    ...
    df = pd.DataFrame(np.zeros(shape=(network_spec["nsamp"], len(network_spec["variable_names"]))),
                      columns=network_spec["variable_names"])
...
def add_noise(df, noise_mean, stdeviation):
    return df + np.random.normal(noise_mean, stdeviation,
            size=(df.shape))

def from_file_to_network_spec(file_name):
    with open(file_name) as f:
        random_seed = f.readline().strip().split("random_seed: ")[1]
        random_seed = int(random_seed) if random_seed != "no_seed" else 42
        ...
        variable_names = f.readline().strip().split("variable_names: ")[1].split(" ")
        nsamp = int(f.readline().strip().split("number_of_samples: ")[1])
        noise_mean, noise_std = map(float, f.readline().strip().split("normal_noise: ")[1].split(" "))

        number_of_generating_functions = int(f.readline().strip().split("number_of_generating_functions: ")[1])
        generating_functions = []
        for i in range(number_of_generating_functions):
            var_name, min_v, max_v = f.readline().strip().split(" ")
            generating_functions.append((var_name, min_v, max_v))
        rest_of_dependencies = map(str.strip, f.readlines())
        dependencies = []
        for dep in rest_of_dependencies:
            end_node, expression = dep.split(" = ")
            dependencies.append((end_node, expression))
        return {"random_seed": random_seed, "variable_names": variable_names, "nsamp": nsamp,
        "noise_mean":noise_mean, "noise_std": noise_std, "generating_functions": generating_functions, "dependencies":dependencies}
...
def get_graphical_network_model_from_dict(network_spec):
    ...
    for end_node, expression in network_spec["dependencies"]:
        ...
        edges.extend(edges_for_dep)
        edges = list(set(edges))
    return CausalGraphicalModel(nodes=network_spec["variable_names"], edges=edges)
